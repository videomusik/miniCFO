...
*/
  ...
  //oscil = waveTable[waveForm*256+index];    // get sample from wave table
  memcpy_P(&oscil1, &waveTable[waveForm1*256+index],1);
  memcpy_P(&oscil2, &waveTable[waveForm2*256+index],1);

  oscil = oscil1*(256-oscMix) + oscil2*oscMix;

  //oscil >> 0;   // sample format for DAC is 12 bit, left justified
  oscil = oscil * gain;
